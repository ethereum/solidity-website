...
export const EventCard: React.FC<EventCardProps> = ({ event, ...props }) => {
  ...
  return (
    <Flex
      ...
      {...props}
    >
      <Image src={imageSrc ? imageSrc : '/assets/default-event-image.png'} h="200" w="full" objectFit="cover" alt="Solidity event image" />
      <Flex
        direction="column"
        px={6}
        py={8}
      ...
      >
        ...
        <Text fontSize="md" color="gray.400">{date}</Text>
        {content && (<Flex direction="column" gap={2}>
          {typeof content === 'string' ? <Text>{content}</Text> : content}
        </Flex>)}
        <Flex direction={['column', null, 'row']} gap={6} mt={4}>
          {links && links.map(({ href, label }) => (
            <ButtonLink href={href} key={href} w="fit-content">{label}</ButtonLink>
          ))}
        </Flex>
      </Flex>
    </Flex>
