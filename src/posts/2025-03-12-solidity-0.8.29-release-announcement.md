...
If you want to give it a try, you can generate ethdebug output by using the CLI: `solc --via-ir --ethdebug|--ethdebug-runtime [..]`. To request the ethdebug artifacts in standard json, select `evm.bytecode.ethdebug` and/or `evm.deployedBytecode.ethdebug` - remember that `viaIR` need to be set.
