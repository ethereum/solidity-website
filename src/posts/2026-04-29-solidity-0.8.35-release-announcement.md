...
Existing builtins like `keccak256` do get [folded](https://en.wikipedia.org/wiki/Constant_folding) by the codegen at the optimization stage; you get the gas savings, but not the type-system benefit of being usable in a comptime context such as a layout specifier.
...
It generates bytecode from the program's [SSA-form](https://en.wikipedia.org/wiki/Static_single-assignment_form) [control-flow graphs](https://en.wikipedia.org/wiki/Control-flow_graph) (the "SSA CFGs", one per function), with a precomputed stack layout that the backend shuffles the EVM stack to match.
...
Stack-too-deep [remained the most commonly reported recurring issue](https://soliditylang.org/blog/2026/04/15/solidity-developer-survey-2025-results/#pain-points) in our [2025 Developer Survey](https://soliditylang.org/survey-2025/), and compilation via IR has long been measurably slower than the evmasm pipeline.
...
[`require(condition, CustomError(...))`](https://soliditylang.org/blog/2024/05/21/solidity-0.8.26-release-announcement/#requires-with-custom-errors) complicates that picture, because a custom error encodes structured data.
